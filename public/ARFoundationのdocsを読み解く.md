...
　AR Foundation は、パッケージ マネージャーから入手できる公式 Unity パッケージです。
...
アクティブな AR 機能によりデバイスの電力消費量が増える可能性があるため、アプリで AR 機能を使用していないときはマネージャーを無効にすることをお勧めします。
...
はじめる
アプリでバウンディング ボックスのTrackingを有効にするには、XR Origin GameObject に AR バウンディング ボックス マネージャー コンポーネントを追加します。シーンに XR Origin GameObject が含まれていない場合は、まずシーンのセットアップ手順に従ってください。

アプリで境界ボックス検出機能が必要ない場合は、AR 境界ボックス マネージャー コンポーネントを無効にして、アプリのパフォーマンスを向上させます。ユーザーのデバイスが境界ボックス検出をサポートしていない場合、AR 境界ボックス マネージャー コンポーネントは実行中に自動的に無効になりますOnEnable。

検出された境界ボックスに応答する
有効になっている場合、AR バウンディング ボックス マネージャー コンポーネントは、フレームごとにXRBounding BoxSubsystemによって報告される変更を取得します。バウンディング ボックスが追加、更新、または削除された場合、関連する情報とともにTracking可能な変更イベントが呼び出されます。
...
シーン内の境界ボックスを視覚化する
デフォルトでは、AR バウンディング ボックス マネージャーは、3D バウンディング ボックスが検出されても、シーン内にテクスチャ メッシュをレンダリングしません。バウンディング ボックスの視覚化を有効にするには、プレハブを AR バウンディング ボックス マネージャーのバウンディング ボックス プレハブとして設定します。
...
除去時に破壊	の場合true、このTracking可能オブジェクトが削除されると、このコンポーネントの GameObject は破棄されます。
境界ボックスのライフサイクル
Tracking可能な AR 境界ボックスには、追加、更新、削除の 3 つのフェーズで構成されるライフサイクルがあります。アプリは、AR 境界ボックス マネージャー コンポーネントのイベントをサブスクライブすることで、AR Session中に検出された 3D 境界ボックスに応答trackablesChangedできます。

追加した
3D バウンディング ボックスが最初に検出されると、バウンディング ボックス マネージャーは AR バウンディング ボックス コンポーネントがアタッチされた新しい GameObject を作成し、イベントを呼び出して、追加されたtrackablesChangedプロパティを介して新しい AR バウンディング ボックス コンポーネントへの参照を渡します。

更新
3D 境界ボックスが追加された後の各フレームで、境界ボックス マネージャーはその境界ボックスの情報を更新する場合があります。

Tracking状態
境界ボックスがデバイス カメラの視野から外れると、境界ボックス マネージャーは境界ボックスを削除する代わりに、Tracking状態をLimitedに設定することがあります。Limited の値は、境界ボックス マネージャーが境界ボックスを認識しているが、現在その位置をTrackingできないことを示します。
...
Tracking画像プレハブ
このプレハブは、参照画像ライブラリから画像が検出されるたびにインスタンス化されます。マネージャーは、インスタンス化されたものにコンポーネントGameObjectが含まれていることを確認します。プロパティを使用してARTrackedImage、検出に使用された参照画像を取得できます。ARTrackedImageARTrackedImage.referenceImage

Tracking状態
Tracking状態には次の 3 つがありますARTrackedImages。

Tracking状態	説明
なし	画像はTrackingされていません。画像が最初に検出されたときの初期状態である可能性があることに注意してください。
限定	Limited画像はTrackingされていますが、それほど効果的ではありません。画像が代わりに考慮される状況は、Tracking基盤となる AR フレームワークによって異なります。LimitedTrackingの原因となる可能性のある例は次のとおりです。
カメラに映らないように画像を隠します。
画像は動画としてTrackingされません。これは、たとえば をmaxNumberOfMovingImages超えた場合に発生することがあります。
Tracking	基盤となる AR SDK は、画像をアクティブにTrackingしていることを報告します。
画像が表示されるタイミングを決定する
画像の可視性を判定する API はありません。通常、Tracking状態がTrackingの場合、画像が表示されていないときは に変わる可能性がLimitedあります。ただし、Tracking状態が 以外の状態になる状況もありますTracked。
...
参照オブジェクト ライブラリを使用する最も簡単な方法は、インスペクターを使用して、Tracking対象オブジェクト マネージャーの参照ライブラリフィールドに参照を保存することです。ただし、次に示すように、実行時に新しい参照オブジェクト ライブラリを作成して使用することもできます。
...
実行時にマネージャーを作成する
実行時にアクティブな GameObject にコンポーネントを追加すると、Unity はすぐにコンポーネントのOnEnableメソッドを呼び出します。ただし、ARTrackedObjectManagernull 以外の参照オブジェクト ライブラリが必要です。ARTracked オブジェクト マネージャーが有効になっているときに参照オブジェクト ライブラリが null の場合、 ARTrackedオブジェクトマネージャーは自動的に無効になります。
...
ARFaceマネージャー。Tracking可能項目が変更されました	このメソッドには、フレーム内で変更されたすべての面のリストが含まれます。
Tracking状態
...
更新
Trackingされたレイキャストが追加された後の各フレームで、AR レイキャスト マネージャーはそのレイキャストの情報を更新する場合があります。次の表に示すように、Trackingされたレイキャストが更新される各フレームで呼び出される 2 つのイベントがあります。
...
ARRaycastマネージャー。Tracking可能項目が変更されました	このメソッドには、フレーム内で変更されたすべてのレイキャストのリストが含まれます。
Tracking状態
...
AR アンカー マネージャー コンポーネントとAR イメージ Tracking マネージャー コンポーネントを使用してシーンを設定します。
AR イメージ Tracking マネージャーのtrackablesChangedイベントを購読します。
AR イメージ Tracking マネージャーが を呼び出すとtrackablesChanged、Transform新しく追加された のコンポーネントが取得されますARTrackedImage。
Trackingされた画像の位置と回転にアンカーを作成します。
アプリで画像Trackingが不要になった場合は、システム リソースを節約するために AR 画像Tracking マネージャーを無効にします。
ポイントクラウドに対するレイキャスト
...
デバイスがアンカーをサポートしていない場合、AR アンカー マネージャー コンポーネントは実行中に無効になりますOnEnable。

アンカーを作成する
AR アンカー マネージャー コンポーネントを有効にすると、次の 4 つの方法でアンカーを作成できます。
...
実行時に AR アンカー コンポーネントが有効になっている場合、シーン内の AR アンカー マネージャーを使用して、自身をアンカーとして追加しようとします。AR アンカー コンポーネントを使用してアンカーを作成する場合は、このアプローチの制限を理解することが重要です。

重要
実行時に AR アンカー コンポーネントを有効にすると、AR アンカー マネージャーに自身をアンカーとして追加するように要求します。この要求は失敗する可能性があり、失敗した場合、AR アンカー コンポーネントはその GameObject を非アクティブ化し、Trackingされなくなります。
...
アンカーライフサイクルイベント
有効になっている場合、AR アンカー マネージャー コンポーネントは、フレームごとにXRAnchorSubsystemによって報告される変更を取得します。アンカーが追加、更新、または削除された場合、AR アンカー マネージャーは関連情報とともにTracking可能な変更イベントを呼び出します。
...
AR アンカー コンポーネントを破棄すると、シーン内の AR アンカー マネージャー コンポーネントを使用して、アンカー Subsystemからコンポーネント自体が削除されます。この方法でアンカーを削除すると、このイベントが呼び出される前にコンポーネントが破棄されているため、次のイベントの削除済みリストにはARAnchorManager.trackablesChangednull コンポーネントが含まれます。ARAnchor

シーン内のアンカーを視覚化する
デフォルトでは、AR アンカー マネージャーは、アンカーが検出されてもシーン内のジオメトリをレンダリングしません。アンカーの視覚化を有効にするには、プレハブを AR アンカー マネージャーのアンカー プレハブとして設定します。
...
除去時に破壊	の場合true、このTracking可能オブジェクトが削除されると、このコンポーネントの GameObject は破棄されます。
アンカーのライフサイクル
Tracking可能な AR アンカーには、追加、更新、削除の 3 つのフェーズで構成されるライフサイクルがあります。ARアンカー マネージャー コンポーネントのイベントをサブスクライブすると、アプリは AR Session中にアンカー ライフサイクル イベントtrackablesChangedに応答できます。

追加した
他のほとんどのTracking可能なタイプとは異なり、アンカーはEnvironment内で自動的に検出されません。アンカーを追加するには、アンカーを作成する必要があります。

保留中
アンカーを作成すると、AR アンカー マネージャーのTracking可能項目の変更イベントが をARAnchor追加済みとして報告するまでに 1 フレームかかることがあります。 が追加されてから を介して追加済みとして報告されるまでの間、アンカーは保留状態になります。アンカーが保留中かどうかを判断するには、 pendingtrackablesChangedプロパティを使用します。

更新
アンカーが追加された後の各フレームでは、AR アンカー マネージャーは、そのアンカーのポーズを更新して、そのアンカーのプラットフォームの表現と一致するようにする場合があります。これらの更新は、アンカーのTransformコンポーネントに自動的に適用されます。

注記
アンカーは、作成後に移動することはできません。アンカーは、TransformAR プラットフォームによってTrackingされている間、AR アンカー マネージャー コンポーネントによって制御されます。

Tracking状態
アンカーがデバイス カメラの視野から外れると、AR アンカー マネージャーはアンカーを削除するのではなく、アンカーのTracking状態を制限に設定する可能性があります。制限の値は、AR アンカー マネージャーがアンカーを認識しているが、現在その位置や回転をTrackingできないことを示します。
...
さらに、すべてのデバイスがメッシュ マネージャーのすべての機能をサポートしているわけではありません。メッシュ マネージャーの一部のプロパティは、特定のプラットフォームでは無視されます。プラットフォーム固有のパッケージ ドキュメントには、各プラットフォームで使用できるメッシュ機能が詳しく記載されています。

シーン内でメッシュを使用する
AR Foundation でメッシュを使用するには、シーンの XR Origin の子 GameObject にARMesh Managerコンポーネントを追加します。

AR メッシュ マネージャー コンポーネント
AR メッシュ マネージャー コンポーネント
...
AR Environmentプローブ マネージャー コンポーネント
Environmentプローブ マネージャーはTracking可能なマネージャーの一種です。
...
飛行機発見パラメータ	平面検出のパフォーマンス調整オプション。
